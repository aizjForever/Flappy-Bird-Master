...
class QNet(object):
    ...
    def build(self):
        ...
        """
        model = Sequential()
        model.add(Dense(self.num_hidden1, init= lecun_uniform(seed = 15381), input_shape=(self.num_inputs,)))
        model.add(Activation('relu'))

        model.add(Dense(self.num_hidden2, init= lecun_uniform(seed = 15381)))
        model.add(Activation('relu'))

        model.add(Dense(self.num_output, init= lecun_uniform(seed = 15381)))
    ...
    def flap(self, input_data):
        ...
        state = np.array([input_data.distY - BIRDHEIGHT])
    ...
    def update(self, last_input, last_choice, last_prediction, crash, scored, playerY, pipVelX):
        ...
        def compute_reward(new_distY,crash,scored,playerY,last_choice):
            if new_distY >= PIPEGAPSIZE - BIRDHEIGHT:
                if last_choice == 1:
                    return -100
                return 180
        ...
        new_distY = last_input.pipeY - playerY - BIRDHEIGHT
        ...
        if crash:
            ...
            new = compute_reward(new_distY,crash,scored,playerY,last_choice) + gamma * np.max(prediction)
        ...
        old_state = np.array([last_input.distY - BIRDHEIGHT])
